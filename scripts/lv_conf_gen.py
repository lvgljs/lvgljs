...
from codegen_utils import ROOT, enum_body, is_stale, write_if_changed

GEN_SCRIPT = "scripts/lv_conf_gen.py"
DEFAULT_LV_CONF_CPP = ROOT / "src/render/native/core/lv_conf/lv_conf.cpp"
DEFAULT_LV_CONF_MANIFEST = ROOT / "scripts/lv_conf_manifest.json"
...
DEFAULT_CPP_LV_SYMBOL_MAP_JSON = ROOT / "scripts/cpp_lv_symbol_mapping.json"
TRANSITION_COMPARE_JSON = ROOT / "scripts/transition_lvgl_compare.json"
...
REG_STYLE_PROP_ASSIGN_RE = re.compile(
    r"^\s*(LV_STYLE_\w+)\s*=\s*lv_style_register_prop\s*\("
)
...
def render_extend_cpp(symbols: list[str]) -> str:
    ...
    for symbol in symbols:
        lines.append(
            f"    {symbol} = lv_style_register_prop(LV_STYLE_PROP_FLAG_NONE);"
        )
    lines += ["}", ""]
    return "\n".join(lines)
...
def build_register_prop_literals(last_built_in: int) -> dict[str, int]:
    """Fake TS literals from lv_style_register_prop order (runtime ids, compile-time only)."""
